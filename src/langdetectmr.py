...
@author: vandana
Important utility for language detection using nltk
...
class LangIdCorpusReader(CorpusReader):
  ...
  LangID corpus reader
	...
	def _read_trigram_block(self, stream):
		...
		'''
		freqs = []
		for _ in range(20):  # Read 20 lines at a time.
...
class LangDetectTwitter(ModifiedMRJob):
  ...
  #def __init__(self, languages=['nl', 'en', 'fr', 'de', 'es']):
	def __init__(self, *args, **kwargs):
		...
		languages = ['fr', 'en', 'ar', 'es', 'de', 'it', 'id', 'pt', 'tr', 'ru', 'nl', 'hi', 'sv', 'fi', 'da', 'pl', 'hu', 'fa', 'he', 'ur', 'th'] 
